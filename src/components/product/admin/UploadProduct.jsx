...
const UploadProduct = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!product.title || !product.subTitle || !product.price || !product.desc || !product.photo.length>0  || !product.sizes.length>0  || !product.color.length>0 ) {
      alert1.current.style.display = "block";
      setalertMessage("Please fill all the fields.");
      return;
    }
      console.log("product photo length is ",product.photo.length);
      console.log("product photo color is ",product.photo.color);
      console.log("product photo sizes is ",product.photo.sizes);
      if (!product.category) {
        ...
        return(
          ...
          <div>
            ...
            <button className="btn btn-primary ms-2" onClick={(e) => {
              setColorLength(colorLength >= 2 ? colorLength - 1 : colorLength);
              let newProduct = product;
          ...
          <div className="mb-3">
            ...
            <button className="btn btn-primary ms-2" onClick={(e) => {
              setImagesLength(imagesLength >= 2 ? imagesLength - 1 : imagesLength);
              let newProduct = product;
